function AvatarUpload (options) {
  ...
  CreateAvatar.prototype = {
    ...
    bindEvent () {
      ...
      AddListener(self.avatarlove, 'touchmove', e => {
        ...
        if (count === 1) {
          ...
          if (place.gw * scale >= position.r * 2 && place.gh * scale >= position.r * 2) {
            ...
            move.x -= (ngw - place.gw) / 2
            move.y -= (ngh - place.gh) / 2
      ...
      AddListener(self.avatarfile, 'click', () => {
        if (self.product) {
          setAjax({
            data: {
              base: self.product,
    ...
    createDesk (picture) {
      ...
      const avatarcrop = document.createElement('div')
      ...
      const canvasdraw = canvasmask.getContext('2d')
